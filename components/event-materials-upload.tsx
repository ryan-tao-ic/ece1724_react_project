...
export function EventMaterialsUpload({
  ...
  // Check if user can manage materials (either as lecturer or creator)
  const canManageMaterials = isUserLecturer || isUserCreator;
  ...
  return (
    <div className="mt-8">
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to remove "{materialToDelete?.fileName}". This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => materialToDelete && handleDelete(materialToDelete.id)}
      ...
      {/* Display existing materials for all logged-in users */}
      {existingMaterials.length > 0 ? (
                ...
                <div className="flex items-center justify-between px-4">
                  <AccordionTrigger className="py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-sm">{material.fileName}</span>
                    </div>
                  </AccordionTrigger>
                  {canManageMaterials && (
                    <Button
        ...
        <div className="mb-6 border border-gray-200 rounded-md p-6 bg-gray-50 text-center">
          <p className="text-sm text-gray-600">No materials available for this event yet.</p>
        </div>
      )}
      
      {/* Upload section - Only shown to lecturers and creators */}
      {canManageMaterials && (
        <>
          <h3 className="text-sm font-medium mb-3">Upload Materials</h3>
